...
暂停恢复场景会先完成配置时长的源端写入，然后立即恢复 CDC。暂停期间选取的延迟采样标记会写入 `state.json`，待 CDC 报告追平后再到目标端读取并计算延迟。`catch_up_timeout` 默认是 10 分钟，它是最大等待时间，不会导致恢复前固定等待满 10 分钟。该阶段的延迟包含暂停时间和恢复追赶时间。
