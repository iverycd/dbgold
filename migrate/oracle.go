...
func OracleGenerateDiffSQL(r *diff.Result) ([]string, error) {
	var sqls []string
	for _, t := range r.AddedTables {
		sqls = append(sqls, oracleCreateTable(t))
	}
	for _, t := range r.DroppedTables {
		sqls = append(sqls, fmt.Sprintf(`DROP TABLE "%s"`, t.Name))
	}
	for _, td := range r.ModifiedTables {
		for _, col := range td.AddedColumns {
			sqls = append(sqls, fmt.Sprintf(`ALTER TABLE "%s" ADD ("%s" %s)`, td.TableName, col.Name, oracleColType(col)))
		}
		...
		for _, cd := range td.ModifiedColumns {
			sqls = append(sqls, fmt.Sprintf(`ALTER TABLE "%s" MODIFY ("%s" %s)`, td.TableName, cd.Column.Name, oracleColType(cd.Column)))
		}
		for _, idx := range td.AddedIndexes {
			sqls = append(sqls, oracleCreateIndex(td.TableName, idx))
		}
		for _, idx := range td.DroppedIndexes {
			sqls = append(sqls, fmt.Sprintf(`DROP INDEX "%s"`, idx.Name))
		}
		for _, fk := range td.AddedForeignKeys {
			sqls = append(sqls, oracleAddForeignKey(td.TableName, fk))
		}
...
func OracleGenerateFullMigrationSQL(src, dst *schema.FullSchema) ([]string, error) {
	var sqls []string
	for _, seq := range dst.Sequences {
		sqls = append(sqls, fmt.Sprintf(`CREATE SEQUENCE "%s" START WITH %d INCREMENT BY %d`, seq.Name, seq.Start, seq.Increment))
	}
...
func OracleGenerateSelectiveSQL(objects *schema.SelectedObjects) ([]string, error) {
	var sqls []string
...
func oracleCreateTable(t schema.Table) string {
	var lines []string
	var pkCols []string
	for _, col := range t.Columns {
		lines = append(lines, "  "+oracleColDef(col))
		if col.PrimaryKey {
...
func oracleColDef(col schema.Column) string {
...
func oracleCreateIndex(table string, idx schema.Index) string {
...
func oracleAddForeignKey(table string, fk schema.ForeignKey) string {
