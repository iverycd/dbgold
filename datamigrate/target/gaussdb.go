...
import (
	...
	ogpq "gitee.com/opengauss/openGauss-connector-go-pq"
)
...
func NewGaussDB(dsn string) (*GaussDBWriter, error) {
	...
	return &GaussDBWriter{db: db}, nil
}
...
func (w *GaussDBWriter) CopyData(ctx context.Context, table string, cols []string, rows [][]interface{}) error {
	...
	stmt, err := tx.PrepareContext(ctx, ogpq.CopyIn(table, cols...))
	if err != nil {
		return err
	}
	defer stmt.Close()
